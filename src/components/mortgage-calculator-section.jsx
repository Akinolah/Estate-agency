...
export function MortgageCalculatorSection() {
  ...
  const [monthlyPITI, setMonthlyPITI] = useState(null); // Removed type
  const [monthlyPrincipalInterest, setMonthlyPrincipalInterest] = useState(null); // Removed type
  const [totalInterestPaid, setTotalInterestPaid] = useState(null); // Removed type
  const [totalPayment, setTotalPayment] = useState(null); // Removed type
  const [amortizationSchedule, setAmortizationSchedule] = useState([]); // Removed type
  ...
  function calculateAmortization(principal, monthlyInterestRate, numberOfPayments, monthlyPI) { // Removed types
     // Calculation uses base currency values
    const schedule = []; // Removed type
